...
/mob/dead/new_player/proc/get_title_html()
	...
	if(SSticker.current_state == GAME_STATE_STARTUP)
		...
		if(!SSticker || SSticker.current_state <= GAME_STATE_PREGAME)
			dat += {"<a id="ready" class="menu_button" href='byond://?src=[text_ref(src)];toggle_ready=1'>[ready == PLAYER_READY_TO_PLAY ? "<span class='checked'>☑</span> ГОТОВ" : "<span class='unchecked'>☒</span> ГОТОВ"]</a>"}
		else
			dat += {"
				<a class="menu_button" href='byond://?src=[text_ref(src)];late_join=1'>ПРИСОЕДИНИТЬСЯ К ИГРЕ</a>
				<a class="menu_button" href='byond://?src=[text_ref(src)];view_manifest=1'>МАНИФЕСТ</a>
			"}
///			dat += {"<a class="menu_button" href='byond://?src=[text_ref(src)];character_directory=1'>CHARACTER DIRECTORY</a>"}
		...
		dat += {"
			<hr>
			<a class="menu_button" href='byond://?src=[text_ref(src)];character_setup=1'>ПЕРСОНАЖ (<span id="character_slot">[uppertext(client.prefs.read_preference(/datum/preference/name/real_name))]</span>)</a>
			<a class="menu_button" href='byond://?src=[text_ref(src)];game_options=1'>НАСТРОЙКИ</a>
			<a id="be_antag" class="menu_button" href='byond://?src=[text_ref(src)];toggle_antag=1'>[client.prefs.read_preference(/datum/preference/toggle/be_antag) ? "<span class='checked'>☑</span> БЫТЬ АНТАГОНИСТОМ" : "<span class='unchecked'>☒</span> БЫТЬ АНТАГОНИСТОМ"]</a>
		...
		<script language="JavaScript">
			...
			var ready_marks = \[ "<span class='unchecked'>☒</span> ГОТОВ", "<span class='checked'>☑</span> ГОТОВ" \];
